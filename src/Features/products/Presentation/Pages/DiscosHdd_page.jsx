...
export const DiscoHddPage = () => {
  const { data: products, error, loading } = useGetProductQueryHook({ category: "Discos duros HDD" });
  ...
  return (
    <div className="flex">
      ...
      <div className="flex-grow p-10 bg-gray-50">
        <h1 className="text-center text-4xl font-semibold mb-12 text-orange-500">Discos duros HDD</h1>
        ...
        {products?.length === 0 || products === undefined ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {products?.map((product) => (
              <Link to={`/discos-durosHDD/${product.id}`} key={product.id} className="relative">
